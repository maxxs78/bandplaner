...
*Änderungen gegenüber Version 1.12: Selbstbediente Konto-Löschung umgesetzt (Abschnitt 6.3) — über „Mein Profil" kann jede Person ihr Konto zweifach bestätigt (E-Mail-Adresse + aktuelles Passwort) und mit deutlicher Warnung vor der Unwiderruflichkeit endgültig löschen. Persönliche Daten werden vollständig entfernt; bandweit geteilte Objekte, an denen die Person nur als Ersteller:in/Hochladende beteiligt war (Termine, Songvorschläge, Finanzeinträge, hochgeladene Dateien, Einladungen), bleiben demgegenüber funktionsfähig erhalten, jedoch ohne jeden Hinweis auf die gelöschte Person. Verweigert wird die Löschung, solange die Person in einer Band die einzige Administratorperson ist. Betroffenenrecht "Löschung" damit von "teilweise" auf "umgesetzt" aktualisiert.*

*Änderungen gegenüber Version 1.14: Song- und Setlisten-Bereich ausgebaut. Songbibliothek (3.3): neues Metadatenfeld „Lead-Gesang/Besetzung"; Suchfeld und Sortierung (Titel/Tempo/Tonart/zuletzt hinzugefügt). Anlageassistent (3.3): Online-Recherche fragt MusicBrainz, Discogs und Spotify jetzt parallel ab und zeigt alle Treffer gebündelt als auswählbare Liste mit Quelle und Cover-Vorschau; die Auswahl ersetzt vorhandene Werte (statt nur leere Felder zu füllen), das Cover ist separat wählbar. Neues abschaltbares Modul „Proben-Tracking" (2.1, 3.3): je Probentermin festhalten, welche Songs geübt wurden (explizit und/oder aus verknüpften Setlisten abgeleitet), mit Historie auf der Song-Detailseite. Setlisten (3.4): Segue-/Medley-Marker mit zusammenhängender Blockdarstellung; Klartext-Export in die Zwischenablage; nachträgliches Synchronisieren der persönlichen Eintrags-Hinweise aus der Song-Notiz (einzeln und gesammelt); Song-Dokumente direkt aus dem Setlist-Eintrag erreichbar. Als geplante Erweiterung notiert: Export einer Setlist als Spotify-Playlist (3.4).*

*Änderungen gegenüber Version 1.15: Übungsmodus (3.12) deutlich ausgebaut — benannte Übungsabschnitte (Loops) lassen sich je Song speichern, bandweit teilen und per Klick wieder aufrufen; eine zum Song synchronisierte Klick-Spur (Grundtempo aus erkanntem/hinterlegtem BPM, automatisch erkannte Downbeat-Phase, manuell feinjustier- und am Song speicherbarer Versatz, Betonung auf Zählzeit 1 aus der Taktart); ein Einzähler mit am Song hinterlegter Schlagzahl (Standard: Taktart-Zähler) vor dem Start und vor jeder Loop-Wiederholung; sowie eine automatische „Tempo-Treppe", die das Tempo je Loop-Durchlauf stufenweise bis zu einem Zielwert anhebt. Zwei neue Song-Stammdatenfelder dafür (Einzähler-Schläge, Klick-Versatz), auch direkt aus dem Übungsmodus setzbar. Übersicht (3.1): personalisierte Karte „Aktion erforderlich" (offene Verfügbarkeits-Rückmeldungen, offene Abstimmungen, Songs ohne Tonart/Tempo), Termin-Karten mit Relativdatum, verknüpfter Setlist und Zusagestatus-Kennzeichnung. Songbibliothek (3.3): Sortierrichtung je Kriterium umkehrbar. Setlisten (3.4): segued Einträge werden als umrahmter Block dargestellt, der Kopieren-Button steht bei Teilen/Drucken.*
...
| Proben-Tracking | aus | zeichnet je Probentermin die geübten Songs auf (siehe 3.3) |
...
- Zentrale, bandweit geteilte Songbibliothek mit Metadaten: Titel, Tonart, Tempo (BPM), Taktart, Dauer, Genre, Lead-Gesang/Besetzung (Freitext), Album und Erscheinungsjahr sowie optionalem Coverbild. Für den Übungsmodus (3.12) zusätzlich: Einzähler-Schläge und Klick-Versatz (beide optional, auch direkt aus dem Übungsmodus setzbar). Die Bibliotheksübersicht bietet ein Suchfeld (Titel/Interpret/Genre/Album) und eine Sortierung (Titel, Tempo, Tonart, zuletzt hinzugefügt) mit umkehrbarer Richtung; Sortierwahl und -richtung werden je Browser gemerkt.
- **Anlageassistent (beim Neuanlegen und beim Bearbeiten eines Songs):**
  ...
  - Online-Recherche auf Knopfdruck: Titel und optional Interpret (aus den bereits erfassten oder per ID3 vorbefüllten Feldern) werden gegen MusicBrainz, Discogs und Spotify **parallel** abgefragt. Alle Treffer werden gebündelt als eine Auswahlliste angezeigt, je Eintrag mit Quelle, Metadaten (Interpret, Album, Jahr, Genre) und einer serverseitig nachgeladenen Cover-Vorschau. Die Auswahl eines Treffers **ersetzt** Titel/Interpret/Album/Jahr/Genre (Felder, die der Treffer nicht mitbringt, bleiben unangetastet) und übernimmt dessen Cover; ein Spotify-Treffer hinterlegt zusätzlich den Track-Link als externen Link. Das Cover ist über eine separate Cover-Leiste unabhängig von der Metadaten-Zeile wählbar. Weil die Übernahme vorhandene Werte ersetzt, dient der Assistent auch zum nachträglichen Bearbeiten. Sämtliche Angaben bleiben vollständig manuell erfassbar bzw. änderbar; ein Cover lässt sich auf der Bearbeiten-Seite jederzeit manuell setzen oder ersetzen.
...
- Persönliche Notizen einzelner Mitglieder je Song (z. B. eigene Spielhinweise), getrennt von bandweiten Informationen. Diese persönliche Notiz kann zusätzlich eine kurze Bühnennotiz, eine Farbe sowie beliebig oft anwendbare Bühnen-Hinweis-Icons enthalten (vollständige Liste und Equipment-Verknüpfung siehe 3.4), die als Vorgabewert übernommen werden, sobald der Song einer Setlist hinzugefügt wird.
- Optionaler, bandweit sichtbarer technischer Hinweis (für FOH/Licht/Technik) je Song, unabhängig von der persönlichen Notiz — erscheint im technischen Ablaufplan-Ausdruck der Setlist (siehe 3.4).
...
- **Proben-Tracking** (je Band abschaltbares Modul, Abschnitt 2.1, Standard aus): Auf jedem Termin vom Typ „Probe" lässt sich festhalten, welche Songs geübt wurden – einzeln aus der Bibliothek zugeordnet (optional mit kurzer Fortschrittsnotiz) und/oder automatisch aus Setlisten übernommen, die mit dem Probentermin verknüpft sind (aus Setlist abgeleitete Einträge sind als solche gekennzeichnet). Die Song-Detailseite zeigt daraus eine chronologische Historie „Geübt bei …". Wird das Modul abgeschaltet, verschwinden die Bereiche, ohne dass Daten gelöscht werden.
...
- Persönliche Kennzeichnung einzelner Setlist-Einträge je Mitglied: individuelle Farbe, kurze Notiz sowie Bühnen-Hinweis-Icons — Umstimmen, Instrumentwechsel (wahlweise direkt mit einem Equipment-Katalogeintrag der Kategorie „Instrumente" verknüpft statt Freitext, siehe 3.9), Equipment (dieselbe Verknüpfung für sonstiges Equipment wie PA, Amps/Pedalboard, Zubehör), Programmwechsel mit Programmnummer, Kapo, Klick, Loop-Pedal, Backing Vocal, Solo, Einzähler, In-Ear sowie freier Hinweis. Jeder Hinweistyp lässt sich je Eintrag beliebig oft anwenden (z. B. zwei Instrumentwechsel innerhalb eines Songs). Nur für das jeweilige Mitglied sichtbar und unabhängig von den bandweiten Songdaten. Trägt der verknüpfte Equipment-Katalogeintrag ein Icon und eine Farbe, lässt sich je Setlist wählen, ob dieses Icon klein im Hinweis-Badge, groß neben Tonart/BPM oder gar nicht dargestellt wird. Diese Farb-, Notiz- und Icon-Vorgaben werden beim Hinzufügen eines Songs aus dessen persönlicher Song-Notiz übernommen; sie lassen sich je Eintrag oder gesammelt für die ganze Setlist nachträglich erneut mit der (ggf. inzwischen geänderten) Song-Notiz synchronisieren (überschreibt die bisherigen Werte des Eintrags).
- **Segue-/Medley-Marker:** Ein Eintrag lässt sich als „Übergang ohne Pause zum nächsten Eintrag" markieren. Aufeinanderfolgende so markierte Einträge werden in Bearbeitungsansicht, Druck und Text-Export als zusammenhängender Block dargestellt; die fortlaufende Nummerierung bleibt davon unberührt.
- Persönliche, freitextliche Anmerkung je Mitglied zur gesamten Setlist, unabhängig von den einzelnen Einträgen.
- Bandweit sichtbarer technischer Hinweis (für FOH/Licht/Technik) auf Ebene der gesamten Setlist sowie je einzelnem Setlist-Eintrag, ergänzend zum technischen Hinweis auf Songebene (siehe 3.3) — auf Eintrags-Ebene editier- und ergänzbar wie die persönlichen Hinweise.
- PDF-/Druckexport der Setlist in konfigurierbaren Layouts: für die Bühne (personalisiert, berücksichtigt die individuellen Farb-, Notiz- und Hinweis-Icon-Einstellungen des jeweiligen Mitglieds, inkl. großer Equipment-Icon-Darstellung falls so gewählt) sowie als eigene, kompakt gesetzte tabellarische Ansicht für Technik/FOH/Licht mit den zusammengeführten technischen Hinweisen aus Song-, Setlist- und Eintrags-Ebene. Zusätzlich ein Klartext-Export der Songliste in die Zwischenablage (zum schnellen Einfügen in Messenger o. ä.), neben dem bestehenden WhatsApp-Teilen-Button.
- Kopieren und Wiederverwenden bestehender Setlisten als Vorlage für neue Termine.
- *Geplante Erweiterung:* Export einer Setlist als Spotify-Playlist bzw. Anlegen einer solchen. Setzt eine Verbindung des jeweiligen Spotify-Kontos je Nutzer voraus (Authorization-Code-Flow) sowie ein Abgleichsschritt zwischen Setlist-Songs und Spotify-Tracks mit Bestätigung bei Mehrdeutigkeit — daher noch nicht umgesetzt.
...
- Verknüpfung der Setlist mit den zugehörigen Song-Dokumenten: Im aufgeklappten Setlist-Eintrag sind die Dateien des Songs (Aufnahmen, Songtexte, Tabs/Noten – im Rahmen der jeweiligen Sichtbarkeit) direkt als Download verlinkt.
...
- **Gig-Details:** Termine vom Typ „Auftritt" bieten zusätzliche Felder für Ankunfts- und Soundcheck-Zeit sowie technische Anforderungen (Freitext, unabhängig von den festen Bühnen/Technik-Angaben des Veranstaltungsorts). Ein Gig-Status (Anfrage, Bestätigt, Abgesagt, Erledigt) verfolgt den Ablauf von der Anfrage bis zum Auftritt selbst; der Abrechnungsstand wird nicht separat gepflegt, sondern automatisch aus den zum Termin verknüpften Gagen-/Kostenanteil-Bestätigungen der Finanzverwaltung abgeleitet (siehe Abschnitt 3.8).
- **Besetzung:** Band-weiter, in den Bandeinstellungen gepflegter Katalog typischer Rollen (z. B. Bass, Lead Guitar, Backing Vocals), optional je Rolle mit einer üblichen Person hinterlegt. Beim Anlegen eines Auftritts wird der Katalog als Startbesetzung in den Termin kopiert und ist dort unabhängig vom globalen Katalog frei änderbar — Rollen hinzufügen, umbenennen oder entfernen, Zuweisung an ein Bandmitglied oder als Freitext für Aushilfen/Deps; dieselbe Person kann dabei mehreren Rollen zugeordnet sein. Änderungen am globalen Katalog wirken sich nicht rückwirkend auf bereits angelegte Auftritte aus.
...
- Optionales Icon aus einem kuratierten Katalog (Instrumente, PA-Technik, Verstärker/Pedale) sowie eine frei wählbare Farbe je Equipment-Eintrag — wird für die Verknüpfung mit den personalisierten Bühnen-Hinweisen in Setlisten genutzt (siehe 3.4).
...
- **Backup & Restore:** Wer die Band verwalten darf (Abschnitt 2), kann jederzeit ein vollständiges Backup der Band als Zip-Archiv herunterladen — sämtliche Datensätze sowie alle hochgeladenen Dateien (Songdateien, Band-Dateien, Cover-/Bandbilder), nicht nur Metadaten. Das Archiv lässt sich später wieder hochladen und **legt dabei immer eine neue Band an** — die hochladende Person wird deren Administrator:in; eine bestehende Band wird dabei nie überschrieben oder ersetzt. Damit dient das Feature zwei Zwecken: Umzug einer Band auf eine andere Bandplaner-Instanz (z. B. Wechsel des Hosting-Anbieters) sowie das Zurückholen eines früheren Stands als neue Kopie (die bisherige Band bleibt unangetastet und kann bei Bedarf manuell entfernt werden). Mitgliedschaften werden beim Wiederherstellen für jede Person übernommen, deren Konto auf dem Zielserver noch existiert (mit ursprünglicher Rolle); Datensätze mit Bezug zu nicht mehr vorhandenen Konten (z. B. persönliche Notizen, Abstimmungen, Verfügbarkeiten, Finanz-Zuweisungen) werden ausgelassen — nach dem Import mit einer zusammenfassenden Übersicht angezeigt. Freigabe-Links öffentlicher Dateien erhalten beim Wiederherstellen neue Tokens, damit alte Links nicht versehentlich auf die neue Band mitwirken.
...
- Gespeicherte Übungsabschnitte: Ein aktuell markierter Loop-Bereich lässt sich benannt speichern (z. B. „Solo", „Bridge"); gespeicherte Abschnitte sind bandweit sichtbar und per Klick sofort wieder aufrufbar (löschbar durch die anlegende Person oder Content-Verwalter:innen).
- Klick-Spur (Metronom): Auf Wunsch zuschaltbar, synchron zum Song. Das Raster ergibt sich aus dem erkannten bzw. hinterlegten Grundtempo; die Phase (Lage der Zählzeit 1) wird beim Analysieren automatisch aus der Beat-Erkennung vorbelegt und lässt sich im Übungsmodus in Millisekundenschritten feinjustieren und — nach Bestätigung — am Song speichern. Die erste Zählzeit jedes Takts wird anhand der am Song hinterlegten Taktart betont.
- Einzähler: Optional werden vor dem Start der Wiedergabe und vor jeder Loop-Wiederholung eine feste Anzahl Klicks vorgezählt (z. B. drei Schläge). Die Schlagzahl ist ein Song-Stammdatenfeld; ohne eigenen Wert wird der Zähler der Taktart verwendet.
- Tempo-Treppe: Automatisches, stufenweises Anheben der Wiedergabegeschwindigkeit bei jedem Loop-Durchlauf (Zielwert, Schrittweite in Prozent und Anzahl Durchläufe je Stufe einstellbar) — zum schrittweisen Herantasten an das Zieltempo einer Passage. Nur bei aktivem Loop-Bereich wirksam.
...
- Bühnentechnik-Integrationen wie MIDI-/DMX-Steuerung, Ansteuerung externer Playback-/Click-Track-Hardware sowie Bluetooth-Fußschalter-Steuerung — der Übungsmodus (Abschnitt 3.12) mit Tempo-/Tonart-Anpassung, Abschnitts-Loop, gespeicherten Übungsabschnitten, songsynchroner Klick-Spur, Einzähler und Tempo-Treppe ist davon unberührt und umgesetzt
...
- **Löschung (Art. 17) — umgesetzt:** Über „Mein Profil" → „Konto löschen" kann jede Person ihr Konto jederzeit selbst und endgültig löschen — zweifach bestätigt durch Eingabe der eigenen E-Mail-Adresse und des aktuellen Passworts, mit deutlicher Warnung vor der Unwiderruflichkeit. Persönliche Daten (Notizen, Bühnen-Hinweise, Verfügbarkeiten/Abwesenheiten, Song-Abstimmungen, persönliches Equipment, Mitgliedschaften in allen Bands) werden dabei vollständig entfernt. Bandweit geteilte Objekte, an denen die Person nur als Ersteller:in oder Hochladende beteiligt war (Termine, Songvorschläge, Finanzeinträge, hochgeladene Dateien, versendete Einladungen), bleiben für die Band funktionsfähig erhalten, jedoch ohne jeden Hinweis auf die gelöschte Person. Die Löschung wird verweigert, solange die Person in einer Band die einzige Administratorperson ist, um die Band nicht unverwaltbar zu machen — die Betroffenen werden dann auf die entsprechenden Bands hingewiesen. Zusätzlich können Administrator:innen weiterhin einzelne Personen aus einer Band entfernen, ohne deren Konto vollständig zu löschen.
