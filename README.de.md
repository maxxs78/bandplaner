...
- **Chat** – genau ein Gruppenchat je Band (keine weiteren Gruppen, kein bandübergreifender Chat) sowie private 1:1-Nachrichten zwischen einzelnen Mitgliedern, strikt je Band getrennt. Gäste können mitchatten. Neue Nachrichten werden per Kurzintervall-Abfrage nachgeladen, ein Punkt am Chat-Reiter zeigt Ungelesenes (Gruppenchat und Direktnachrichten gemeinsam) an. Je Band ein-/ausschaltbar; Chat-Inhalte sind Teil des Band-Backups.
...
- **Band-Backup & -Restore** – ein Band-Admin kann ein vollständiges Backup der Band (alle Daten und hochgeladenen Dateien) als `.zip` herunterladen und später wieder herstellen – als Kopie auf demselben Server oder auf einer anderen Bandplaner-Instanz zum Umzug der Band. Das Wiederherstellen legt dabei immer eine neue Band an, nichts wird überschrieben.
...
Die vollständige, detailliertere fachliche Spezifikation – inklusive geplanter, noch nicht umgesetzter Erweiterungen wie Umfragen/To-Dos und KI-gestützten Setlist-Vorschlägen – steht in [Funktionsspezifikation-Bandplaner.md](Funktionsspezifikation-Bandplaner.md).
...
| Chat | aus | ein Gruppenchat je Band plus private 1:1-Nachrichten, Live-Update per Polling |
