...
export function getDeleteAccountSchema(t: (key: string) => string) {
  return z.object({
    ...
    confirmEmail: z.string().min(1, t("confirmEmailRequired")),
  });
}
...
export function getSongSchema(t: (key: string) => string) {
  return z.object({
    ...
    countInBeats: z.coerce.number().int().min(0).max(16).optional().or(z.literal("")),
    clickOffsetMs: z.coerce.number().int().min(-2000).max(2000).optional().or(z.literal("")),
