  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-foreground">Setlisten</h1>
        {canCreate && (
      ...
      <div className="mt-4 space-y-2">
        {setlists.length === 0 && <Card className="text-sm text-muted">Noch keine Setlisten.</Card>}
        {setlists.map((s) => (
            ...
            <Card className="flex items-center justify-between transition hover:border-primary">
              <div>
                ...
                <p className="text-sm text-muted">
                  {s._count.items} {s._count.items === 1 ? "Song" : "Songs"}
                  {s.event ? ` · verknüpft mit „${s.event.title}“` : ""}
                </p>
              </div>
            </Card>
          </Link>
