...
export default async function AvailabilityPage({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="font-semibold text-foreground">{t("matrixTitle")}</h2>
        <p className="mt-1 text-sm text-muted">{t("matrixSubtitle")}</p>
                  ...
                  {events.map((e) => (
                    <th key={e.id} className="border-b border-border px-2 pb-2 text-center font-medium text-muted">
                      <Link href={`/bands/${bandId}/calendar/${e.id}`} className="hover:text-primary">
                        {format.dateTime(e.startsAt, { day: "2-digit", month: "2-digit" })}
                        <br />
                ...
                {members.map((m) => (
                  <tr key={m.id}>
                    <td className="border-b border-border py-2 pr-4 text-foreground">
                      {m.user.name}
                      {m.userId === user.id && <span className="text-muted">{t("you")}</span>}
                    </td>
                    {events.map((e) => {
                      ...
                      return (
                        <td key={e.id} className="border-b border-border px-2 py-2 text-center">
                          {!isParticipant ? (
                            <span className="text-muted/40" title={t("notApplicableTitle")}>
                              {t("notApplicable")}
                            </span>
                          ) : a ? (
                            <span
      ...
      <div>
        <h2 className="font-semibold text-foreground">{t("personalAbsences")}</h2>
        <p className="mt-1 text-sm text-muted">{t("personalAbsencesSubtitle")}</p>
        <Card className="mt-3">
        ...
        <div className="mt-3 space-y-2">
          {absences.length === 0 && (
            <p className="text-sm text-muted">{t("noAbsences")}</p>
          )}
          {absences.map((a) => (
            <Card key={a.id} className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted">
                  {format.dateTime(a.startDate, { dateStyle: "medium" })} –{" "}
                  {format.dateTime(a.endDate, { dateStyle: "medium" })}
                  {a.reason ? ` · ${a.reason}` : ""}
                </p>
              </div>
              <DeleteButton
